---
updated: 2026-06-11
updateNote: 工場停止条件の変更（レビュー件数では止めない）+ Inboxを4セクション構成へ
---
...
| Inbox | `/decide` | 4セクション構成。① 今日の判断=工場停止要因のみ（危険判断/方針選択/人間作業・最大3件・約3分） ② レビュー=検収（放置しても工場は止まらない） ③ Epic候補=実行許可（放置可能） ④ AI保留=件数のみ。内部分類・内部IDは「詳細を見る」内のみ |
