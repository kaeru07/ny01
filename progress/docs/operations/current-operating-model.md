---
updated: 2026-06-11
updateNote: Inboxを「今日の判断」（社長向け承認アプリ）に全面リデザイン
---
...
| Inbox | `/decide` | 「今日の判断」。カードは3種類（作業結果の確認 / 次の作業 / Goal紐付け）だけ。はい・いいえ・あとで を選ぶだけで、内部分類（承認 / needs_human / suggested 等）はAIが裏で管理。内部IDは「詳細を見る」内のみ |
...
| 画面上の分類 | 内部の対応 | ボタン |
|---|---|---|
| 作業結果の確認 | needs_human の作業履歴 / レビュー系承認 | 問題なし / 修正する |
| 次の作業 | suggested 候補 / 実行系承認 | はい / あとで / やめる |
| Goal紐付け | goalId 未設定の Epic | 目標を選んで紐付ける / 不要 |
