...
function buildAiPrompt(seeds: ProposalSeed[], observation?: MarketObservationSeed): string {
  ...
  return [
    ...
    JSON.stringify([{
      ...
      spec: '詳細仕様。機能、画面遷移、データ、通知、オフライン、技術前提などの要約。審査に必要な要素（プライバシー・課金審査要件）があれば含める。',
      mvpScope: '最初のストア審査に出せるMVP範囲。画面、必須機能、後回し機能を1〜3文で書く。',
      difficulty: 'medium',
      externalApis: ['RevenueCat', 'Firebase', 'MapKit'],
      initialGoalDraft: 'Codexが最初に作るべきGoal案。プロトタイプではなくApp Store提出可能品質までの実装計画として書く（基盤→機能完成→品質仕上げ→公開準備の4フェーズ想定）。初期画面、データモデル、最小保存/課金スタブなどを1〜3文で書く。',
      decisionPoints: [
    ...
    '- initialGoalDraft はCodexへ渡す「最初に何を作るか」の初期Goal案を1〜3文で書く。プロトタイプではなくApp Store提出可能品質までの実装計画として書く（基盤→機能完成→品質仕上げ→公開準備の4フェーズ想定）。',
    '- spec には審査に必要な要素（プライバシー・課金審査要件）があれば含める。',
