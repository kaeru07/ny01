...
const SECTIONS = [
  ...
  {
    title: '基本フロー',
    body: `1. 朝会（/morning）で今日の作業候補を確認・承認する
2. ToDo管理の「今日の作業に追加」でキューに入れる
3. キュー（/queue）でプロンプトをコピーする
4. Claude Codeに貼って作業させる
5. Claude Codeの完了報告を受け取る
6. 問題なければ done にする
7. 未完了があれば次ToDoを作る
8. blockedなら理由と解除方法を書く`,
  },
  {
    title: '各ページの役割',
    body: `朝会: 今日の作業を計画する・推薦候補を確認・チェックリストをこなす
ダッシュボード(/): キューの状態・全体の件数・最近のログを見る
...
・朝会に戻れない → 下部ナビの「朝会」を押す / ダッシュボードは「/」へ直接アクセス
