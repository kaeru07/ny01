...
const FAQ: Array<{ q: string; a: string }> = [
  ...
  { q: '自走化・最優先って何？', a: 'North Starの「AI工場OS自走化」（goal-ai-factory-os）を止めないためのアンカーです。Goal単体は実行されないため、配下の「AI工場オペレーションセンター」EpicにP0・Factory対象・検証可能な完了条件を持たせ、表示キューとFactory実行の両方で同じ最優先候補として扱います。ただし危険操作・承認待ち・ブロック中はこれまで通り自動実行しません。' },
  ...
  { q: '自分でゴールを追加するには？', a: '目標タブ（/goal-planner）から手動で追加できます。方法は2つ：(1)「ゴールを直接追加」=タイトル・案件・優先度・状態・指標・今(current)・目標(target)などを項目入力して1件追加。進捗%は「今/目標」で決まります。(2)「JSONで一括追加」=ゴール定義のJSON（projectId/goalTitle/phases/todos）を貼り付けて検証・一括登録。以前あった「分解プロンプト生成」は廃止しました（直接追加とJSON追加だけになりました）。', },
  { q: 'AIが新しいゴールを提案してくるのは？', a: '自動実行の最初に、AI工場が「日々の調査結果」（ニュースアプリのAIツール調査など）を見て、効果がありそうな・やった方が良いことを「次に目指すべきゴール」として1〜3件提案します（例: 評価の高い新ツールを「○○を試す」「○○を調査する」、Fableを試す など）。提案ゴールは「今日の判断」（Inbox）の上に「🎯 ゴール承認」として出ます。「承認する」を押すと次回以降の自動実行の対象になり、達成まで自動で進みます。「やめる」で候補から外れます。承認するまで勝手に実行されることはありません。ゴールは優先順の高いものから処理され、達成して承認待ちが減ると、次の自動実行でまた調査から新しい候補が補充されます。承認待ちが3件たまると、それ以上は提案しません（先に承認/却下してください）。', },
