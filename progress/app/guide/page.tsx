...
const FAQ: Array<{ q: string; a: string }> = [
  ...
  { q: '自分でゴールを追加するには？', a: '目標タブ（/goal-planner）から手動で追加できます。方法は2つ：(1)「ゴールを直接追加」=タイトルだけで追加できます（案件・説明は任意）。優先度や状態などの細かい設定は不要にしました。(2)「JSONで一括追加」=ゴール定義のJSON（projectId/goalTitle/phases/todos）を貼り付けて検証・一括登録。以前あった「分解プロンプト生成」は廃止しました（直接追加とJSON追加だけです）。', },
