...
const FAQ: Array<{ q: string; a: string }> = [
  ...
  { q: '「今日の判断」には何が入る？', a: '止める必要がある3種類だけが入ります。①危険な操作（課金・本番DB・認証情報・外部公開・破壊的操作）②方針を選ぶ判断（複数案から人間が選ぶべきもの・アプリ作成前の必須方針）③手詰まり／完了の確認（前回失敗で止まった作業をどうするか、達成したゴールを完了にしてよいか）。品質レビュー（作業の出来ばえの確認）は「今日の判断」には入れず、自動実行も止めません。レビューは時間のあるときに「レビュー」タブで任意に見るものです。', },
  { q: 'AIの作業は誰がレビューする？完了はどう決まる？', a: '完了の正本は「doneCriteria（完了条件）を満たしたか」の機械判定です。各作業でtsc（型）・build・lint・危険語チェックが自動で走り、これが通って完了条件を満たせば完了になります。レビューは完了の条件ではなく、あとから品質を確認するためのものです。より深い設計レビュー（Fableレビュー）は、あなたが「レビューして」と指示したときだけ実行します（毎回自動では回しません）。これにより、レビュー待ちで自動実行全体が止まることはありません。', },
  { q: '失敗した作業はどう扱われる？', a: 'レビューには入れません。原因で自動的に分かれます。①一時的な原因（Claudeの利用上限・一時的なネットワーク等）は自己解決するので、そのままキューに戻して再実行します。②実エラーや原因不明のものだけ「今日の判断」に出て、「再試行／原因を調べる／中止／保留」から選べます。失敗しただけで自動実行全体が止まることはありません。', },
  ...
  { q: '1サイクルで複数の作業を回すには？', a: 'Legacy内の「自動化」（/automation）で「1サイクルの深掘り回数」を1回にします。同じ大きな作業（Epic）を1回実行すると次のEpicへ進むため、1サイクルで複数の異なる作業を回せます。2〜3回にすると、同じEpicをその回数まで続けて深掘りします。これは1サイクル全体の実行件数ではなく、同じEpicを連続して実行する上限です。' },
  ...
  { q: '目標・自動実行・ゴール進行ボードはどう使い分ける？', a: '「目標」（/goal-planner）はGoalと補助todoの作成・編集・並び替えをする管理画面です。「自動実行」（/queue）はAIが次に何をやるか、実行順やpin/保留/対象外を操作する実行画面です。「ゴール進行ボード」（/goal-dashboard）は、Goal配下のEpicとRunを見て、実際に自動実行が動いているか、直近いつ動いたか、7日以上停滞していないかを確認する閲覧専用画面です。', },
  ...
  { q: '自動実行の結果はメールで届く？', a: 'はい。自動実行（11/14/16/23時と起動時）が1回終わるたびに、その回のレポートメールが1通届きます。中身は①いま自動実行を止めている項目・不具合（赤・最優先）②システム自己点検③App Storeリリース準備度④ゴール別の進捗・問題・やること⑤直近のスケジュール実行⑥できるようになったこと・完了した作業⑦改善事項・次にやること、です。件名に「要対応◯件 / 完了◯・変更◯ファイル」が出るので、要対応0件なら開かなくて大丈夫です。Progressを開かなくても状況が分かる通知で、判断そのものは今までどおりProgressのInboxで行います。', },
  { q: 'レポートメールに付いてくるMarkdownは何？', a: 'ChatGPTなど外部AIにそのまま貼れる「状態レビュー依頼」のファイルです（progress-review-日付.md）。要対応の優先順位・危険判断の承認可否・停滞ゴールの継続/保留/破棄・仕組みの穴、といった観点でレビューを頼める依頼文と、現在状態のスナップショットが入っています。外部AIの指摘を採用するときは、これまでどおり人間がProgress側（レビュータブの「修正する」＋修正指示、またはInboxへの起票）に写して反映します。', },
  { q: 'レポートメールが届かない・止めたいときは？', a: 'メールは自動実行のあとに付随して送るだけなので、届かなくても自動実行は正常に動き、作業は止まりません。送信の成否は自動実行のログ（/root/company/logs/factory-schedule.log の「report email:」行）で確認できます。設定変更（宛先・停止）は運用側の設定作業なので、必要なときはAIに依頼してください。', },
  { q: '自動で何が動いたか確認したい', a: '「実行履歴」（/logs）または「状況」（/activity）で、最近AI工場が自動で動かした作業（結果・日時）を一覧で確認できます。まとまった自動実行サマリーは運用ページ上部の「自動実行レポート」タブでも見られます。（以前は「今日の判断」の下にも履歴を出していましたが、重複のため撤去しました）', },
...
export default async function OperationsGuidePage({ searchParams }: { searchParams?: Record<string, string | string[] | undefined> }) {
  ...
  // 最後の「メール報告」= 自動実行1回ごとのレポートメール送信（ボトルネックにはならない通知ステップ）
  const factoryStages = ['目標', '大きな作業', 'AI作業', 'レビュー', '学習', '次の作業', 'メール報告']
  ...
  return (
    <div className="space-y-4 px-4 pb-5 pt-4">
      ...
      <Slide n={2} title="今日の流れ" accent="amber" lead="朝と夜、それぞれ数分で終わります。自動実行が1回終わるたびにレポートメールが届くので、そこから始めれば大丈夫です。">
        <div className="grid grid-cols-2 gap-2">
          <div className="rounded-xl border border-amber-200 bg-white p-2 dark:border-amber-900/60 dark:bg-gray-900">
            ...
            <FlowDiagram steps={['レポートメールを見る', '司令塔を開く', 'Inboxを開く', '判断する', '必要ならレビュー用コピー', '終了'].map((label) => ({ label }))} />
          </div>
          <div className="rounded-xl border border-indigo-200 bg-white p-2 dark:border-indigo-900/60 dark:bg-gray-900">
            ...
            <FlowDiagram steps={['レポートメールを見る', '司令塔を開く', 'Inboxを開く', 'おすすめ次作業を確認', '必要ならレビュー用コピー', '終了'].map((label) => ({ label }))} />
          </div>
        </div>
        <p className="mt-3 text-[11px] leading-relaxed text-gray-500 dark:text-gray-400">
          レポートメールは自動実行（11/14/16/23時と起動時）が1回終わるたびに1通届きます。件名に「要対応◯件 / 完了◯・変更◯ファイル」が出るので、要対応が0件なら開かなくても大丈夫です。急ぐものがあればメールの先頭に赤で出るので、そこからProgressのInboxへ進んでください。
        </p>
      </Slide>
      ...
      <Slide n={3} title="AI工場の流れ" accent="gray" lead="すべての作業はこの順番でぐるぐる回ります。">
        <LoopDiagram
          steps={factoryStages.map((stage) => {
            ...
            return {
              label: stage,
              icon: stage === '目標' ? '🎯' : stage === 'レビュー' ? '📝' : stage === '学習' ? '🧠' : stage === 'メール報告' ? '📧' : '⚙️',
              highlight: hit,
              sub: hit
                ? `現在 ${bottleneck!.text} ⚠ ここがボトルネックです`
                : stage === 'メール報告'
                  ? '自動実行1回ごとに結果をメールで送ります'
                  : undefined,
            }
          })}
        />
        ...
        <p className="mt-3 text-[11px] leading-relaxed text-gray-500 dark:text-gray-400">
          最後の「メール報告」は、自動実行が1回終わるたびに結果をあなたのメールへ送るステップです（要対応・自己点検・ゴール別の進捗・できるようになったこと・改善事項の1通）。外部AIレビュー用のMarkdownも添付されます。メール送信に失敗しても自動実行そのものは成功扱いのままで、次の作業は止まりません。
        </p>
