...
const FAQ: Array<{ q: string; a: string }> = [
  ...
  { q: 'AIが新しいゴールを提案してくるのは？', a: '自動実行中にやる作業が無くなると、AI工場が「次に目指すべきゴール」を1〜3件提案します。提案ゴールは「今日の判断」（Inbox）の上に「🎯 ゴール承認」として出ます。あなたが「承認する」を押すと、そのゴールは次回以降の自動実行の対象になり、達成まで自動で進みます。「やめる」を押すと候補から外れます。承認するまで勝手に実行されることはありません。承認待ちが3件たまると、それ以上は新しい提案をしません（先に承認/却下してください）。', },
