...
const FAQ: Array<{ q: string; a: string }> = [
  ...
  { q: '目標（Goal Planner）と自動実行キューの関係は？', a: '同じデータを見ています。「目標」タブの各カードに、その目標に紐づくキューの要約（次回候補・実行可能・判断待ち・レビュー待ち・候補外・最新作業・次にやること）が出て、カードの「自動実行キューを見る」を押すとその目標で絞り込んだ自動実行キューの詳細に飛びます。目標タブ＝俯瞰、自動実行タブ＝詳細、で件数は一致します。', },
