...
export default async function GoalDashboardPage() {
  ...
  const executableGoalIds = new Set(queue.executable.map((item) => item.goalId).filter(Boolean))
        ...
        {activeWithAch.length === 0 ? (
          ...
          <ul className="mt-3 space-y-4">
            {activeWithAch.map(({ g, ach }) => {
              const row = goalProgressById.get(g.id)
              const hasExecutable = (row?.executable ?? 0) > 0 || executableGoalIds.has(g.id)
              const todos = g.todos
              const done = todos.filter(isTodoComplete).length
              const stuckCount = todos.filter((todo) => !isTodoComplete(todo) && !hasExecutable).length
              return (
                <li key={g.id} className="rounded-lg border border-gray-100 bg-gray-50/60 p-3 dark:border-gray-800 dark:bg-gray-950/30">
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-1.5">
                        <span className="min-w-0 break-words text-[12px] font-semibold text-gray-800 dark:text-gray-100">{g.title}</span>
                        <span className="rounded bg-gray-100 px-1.5 py-0.5 text-[10px] font-bold text-gray-600 dark:bg-gray-800 dark:text-gray-300">todo {done}/{todos.length} 完了</span>
