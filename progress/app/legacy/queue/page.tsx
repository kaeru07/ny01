...
export default async function LegacyQueuePage() {
  ...
  return (
    <div className="px-4 pt-6 pb-4 space-y-5">
      <header>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">旧: 今日の作業順番</h1>
        <p className="text-sm text-gray-400 dark:text-gray-500 mt-0.5">
          {activeItems.length} 件待機中{hasManualOrder ? ' · 手動順序あり' : ''}
        </p>
        <Link href="/queue" className="mt-2 inline-block text-xs font-semibold text-blue-600 hover:underline dark:text-blue-400">
          Epicベースの自動実行キューへ
        </Link>
      </header>
