...
- `## トピック一覧` が無い、または Topic を 1 件も抽出できない場合でも、`## 記事本文` があれば **記事レイアウト（結論カード + 記事本文 + 残りセクション「調査メモ」）で表示**する（2026-07-19〜）。
- Topic も `## 記事本文` も無い場合は **従来の全文 MarkdownView へ自動 fallback** する（旧フォーマット互換）。
